...
$who = $null
try {
    $whoOutput = vercel whoami 2>$null
    if ($whoOutput) { $who = ($whoOutput | Out-String).Trim() }
} catch {
    $who = $null
}
if (-not $who) {
    ...
    vercel login
    $who = ((vercel whoami) | Out-String).Trim()
}
...
if ($missing.Count -gt 0) {
    $missList = $missing -join ", "
    Write-Err "Missing in .env.local: $missList"
    exit 1
}
...
function Set-VercelEnv {
    param([string]$name, [string]$value, [string[]]$environments)
    foreach ($envName in $environments) {
        ...
        if ($LASTEXITCODE -ne 0) {
            Write-Warn ("  Couldn't set " + $name + " for " + $envName)
        }
    }
}
...
$prodUrl = $null
$urlMatches = [regex]::Matches($deployOutput, 'https://[a-zA-Z0-9.\-]+\.vercel\.app')
foreach ($m in $urlMatches) {
    if ($m.Value -notlike '*vercel.com*') {
        $prodUrl = $m.Value
    }
}
